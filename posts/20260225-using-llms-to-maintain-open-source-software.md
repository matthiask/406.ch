...
A somewhat related thing happened with the too-big-to-fail banks: There, the people at the top were even compensated with [golden parachutes](https://en.wikipedia.org/wiki/Golden_parachute) at the end.
...
So, we can produce more code, more features faster than before. Contrary to what people in my LinkedIn feed say that's clearly not an obviously good thing. But is it a race to the bottom where we drown in LLM-generated slop in quantities impossible to maintain? It doesn't feel like that. I don't think tools are generally neutral, but it's certainly the case that we have coding agents which can be run locally with reasonable resource usage (at least during inference, certainly not during training) and which do help close tickets faster, answer user inquiries faster and produce prototypes quicker.
